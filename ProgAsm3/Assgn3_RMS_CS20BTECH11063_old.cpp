...
{
    int min_timeperiod = INT_MAX;
    // Using iterators to find the minimum time period task
    vector<TaskProcess>::iterator it = ReadyQueue.begin();
    vector<TaskProcess>::iterator min_it;
    for (it = ReadyQueue.begin(); it != ReadyQueue.end(); it++)
    {
        if (it->task_period < min_timeperiod)
        {
            min_timeperiod = it->task_period;
            min_it = it;
        }
    }

    TaskProcess returnProcess(min_it->task_id, min_it->task_time, min_it->task_period, min_it->task_arrivaltime);
    returnProcess.isInterrupted = min_it->isInterrupted;
    // Remove this task from the Ready Queue
    ReadyQueue.erase(min_it);
...
int main()
{
    ...
    while (ListTaskProcesses.empty() == false)
    {
        ...
        while (it != ReadyQueue.end())
        {
            ...
            {
                num_deadline_miss++;
                total_waiting_time += current_time - it->task_arrivaltime - it->task_time;
                missed_it = it;
                output_file_log << "Task ID: " << missed_it->task_id << " missed deadline at time: " << current_time << endl;
        ...
        {
            num_deadline_miss++;
            total_waiting_time += current_time - PrevRunningProcess.task_arrivaltime - PrevRunningProcess.task_time;
        ...
        {
            TaskProcess newTask = ListTaskProcesses.top();
        ...
        if (ReadyQueue.empty() == false || PrevRunningProcess.task_id > 0)
        {
            ...
            // If uptill now the CPU was Idle then it is made to false
            if (IsIdle == true)
            {
                IsIdle = false;
            ...
            // If the process can be completed in one time slice then it is completed
            ...
            while (isCompleted == true)
            {
                ...
                output_file_log << "Task ID: " << runningProcess.task_id << " completed at time: " << current_time << endl;
                total_waiting_time += current_time - runningProcess.task_ideal_finish_time;
