...
void *testCS(void *param)
{
    ...
    for (int i = 0; i < k; i++)
    {
        ...
        waiting_array[thread_id - 1] = true;
        int key = 1;
        ...
        // Bounded CAS lock
        // int a = 0, b = 1;
        while(waiting_array[thread_id - 1] == true && key == 1)
        {
            int a = 0, b = 1;
            // key = lock_guard_.compare_exchange_weak(a, b);
            // key = atomic_compare_exchange_weak(&lock_guard_, &a, b);
            if(atomic_compare_exchange_weak(&lock_guard_, &a, b))
            {
                key = 0;
            }
        }
        waiting_array[thread_id - 1] = false;
        ...
        int j = (thread_id) % n;
        while(j != thread_id - 1 && !waiting_array[j])
        {
            j = (j + 1) % n;
        }
        if(j == thread_id - 1)
...
int main()
{
    ...
    fprintf(output_file_ptr, "Bounded CAS Output\n");
    ...
    output_file_log << "The Average Wait Time for Bounded CAS n: " << n << " is " << GlobalWaitTime / (n * k * 1.0) << endl;
    output_file_log << "The Worst Wait Time for Bounded CAS n: " << n << " is " << WorstWaitTime << endl;
