...
std::atomic_flag lock_guard_ = ATOMIC_FLAG_INIT;
...
void *testCS(void *param)
{
    ...
    for (int i = 0; i < k; i++)
    {
        ...
        // while (true)
        // {
        //     if (atomic_flag_test_and_set(&lock_guard_))
        //     {
        //         break;
        //     }
        // }
        while(atomic_flag_test_and_set(&lock_guard_));
...
int main()
{
    ...
    fprintf(output_file_ptr, "TAS Output\n");
    ...
    output_file_log << "The Average Wait Time for TAS n: " << n << " is " << GlobalWaitTime / (n * k * 1.0) << endl;
    output_file_log << "The Worst Wait Time for TAS n: " << n << " is " << WorstWaitTime << endl;
